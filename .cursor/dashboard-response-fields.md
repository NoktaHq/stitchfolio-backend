...
- **Last 7 days**: `completed` = count of tasks with `is_completed = true` and `completed_at >= sevenDaysAgo`. `total` = count of all tasks with `created_at >= sevenDaysAgo`. **Percent** = `(completed / total) * 100` (0 if total is 0).
...
- **Last 30 days**: `completed` = tasks with `completed_at` in the requested `from`–`to` range and `is_completed = true`. `total` = tasks with `created_at` in the same range. **Percent** = `(completed / total) * 100`.
...
| Field | Calculation |
|-------|-------------|
| **overdueTasks** | Tasks where `is_completed = false`, `due_date < today` (midnight). Channel + active; optional assignee filter. |
| **dueToday** | Tasks where `is_completed = false`, `due_date` in [today 00:00, tomorrow 00:00). |
| **dueNext7Days** | Tasks where `is_completed = false`, `due_date` in [tomorrow 00:00, today + 7 days). |
| **incompleteByAssignee** | For each `assigned_to_id`, count of tasks with `is_completed = false`. User names resolved from `User`. |
| **highPriorityIncomplete** | Tasks with `is_completed = false` and `priority IS NOT NULL AND priority > 0`. |
| **upcomingReminders** | Tasks with `reminder_date` in the next 24–48 hours from now (inclusive). |
| **completionRate** | **Last 7 days**: `completed` = tasks with `completed_at >= sevenDaysAgo` and `is_completed = true`; `total` = tasks with `created_at >= sevenDaysAgo`. **Last 30 days**: same with `thirtyDaysAgo`. **Percent** = `(completed / total) * 100`, 0 if total is 0. |
| **recentCompletions** | Last 10 tasks with `is_completed = true` and `completed_at IS NOT NULL`, ordered by `completed_at DESC`. Optional assignee filter. |
...
| **taskCompletionInPeriod** | **Last 7 days**: empty. **Last 30 days**: `completed` = tasks with `completed_at` in `[from, to]` and `is_completed = true`; `total` = tasks with `created_at` in `[from, to]`; **Percent** = `(completed / total) * 100`. |
